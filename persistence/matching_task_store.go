...
func (d *MatchingTaskStore) UpdateTaskQueue(
	...
	if request.TaskQueueKind == enumspb.TASK_QUEUE_KIND_STICKY {
		...
		params.Add(table.ValueParam("$expire_at", types.OptionalValue(types.TimestampValueFromTime(ToYDBDateTime(request.ExpiryTime.AsTime())))))
	} else {
...
`)
	...
	for _, t := range request.Tasks {
		...
		if t.ExpiryTime != nil {
			taskFields = append(taskFields, types.StructFieldValue("expire_at", types.OptionalValue(types.TimestampValueFromTime(ToYDBDateTime(t.ExpiryTime.AsTime())))))
		} else {
