...
func (d *MutableStateTaskStore) completeTasksInIDRangeSlow(ctx context.Context, shardID int32, categoryID int, inclusiveMinTaskID, exclusiveMaxTaskID int64) error {
	template := d.client.AddQueryPrefix(`
DECLARE $shard_id AS uint32;
	...
	for {
		more, err := func() (more bool, err error) {
			res, err := d.client.Do(ctx, template, table.SerializableReadWriteTxControl(table.CommitTx()), table.NewQueryParameters(
				...
				table.ValueParam("$limit", types.Int32Value(slowDeleteBatchSize)),
			))
			if err != nil {
				return false, err
			}
...
`)
	...
	if err != nil {
		if xydb.IsIntermediateDataMaterializationExceededSizeLimitError(err) {
...
func (d *MutableStateTaskStore) completeTasksInVisibilityTSRangeSlow(ctx context.Context, shardID int32, categoryID int, inclusiveMinVisibilityTS, exclusiveMaxVisibilityTS time.Time) error {
	template := d.client.AddQueryPrefix(`
DECLARE $shard_id AS uint32;
DECLARE $task_category_id AS Int32;
