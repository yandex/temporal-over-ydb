...
func scanTask(client *conn.Client, res result.Result) (*task, error) {
	var encoding string
	var encodingType conn.EncodingTypeRaw
	var encodingScanner named.Value
	if client.UseIntForEncoding() {
		encodingScanner = named.OptionalWithDefault("task_encoding", &encodingType)
	} else {
		encodingScanner = named.OptionalWithDefault("task_encoding", &encoding)
	}
	...
	if client.UseIntForEncoding() {
		t.encoding = enumspb.EncodingType(encodingType).String()
	} else {
		t.encoding = encoding
	}
	return t, nil
...
func scanTaskQueue(client *conn.Client, res result.Result) (*taskQueue, error) {
	var encoding string
	var encodingType conn.EncodingTypeRaw
	var encodingScanner named.Value
	if client.UseIntForEncoding() {
		...
		encodingScanner = named.OptionalWithDefault("task_queue_encoding", &encoding)
	}
	...
	if client.UseIntForEncoding() {
		tq.encoding = enumspb.EncodingType(encodingType).String()
	} else {
...
func scanTaskQueueUserData(client *conn.Client, res result.Result) (*taskQueueUserData, error) {
	var encoding string
	var encodingType conn.EncodingTypeRaw
	var encodingScanner named.Value
	if client.UseIntForEncoding() {
